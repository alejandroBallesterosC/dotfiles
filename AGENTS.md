...
`claude-code/CLAUDE.md` and `claude-code/docs/` are symlinked to `~/.claude/CLAUDE.md` and `~/.claude/docs` — global instructions and reference docs applied to every Claude Code session, regardless of project.

`claude-code/agents/` and `claude-code/skills/` are symlinked to `~/.claude/agents` and `~/.claude/skills`, kept intentionally empty (each holds only a `.gitkeep`) — custom agents/skills live in a separate private repo and are not synced here.
