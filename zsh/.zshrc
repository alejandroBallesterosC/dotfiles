...
# Vertex AI settings
# Workload Identity Federation credential config: contains NO secrets, only the IMDS URLs the
# google auth libraries read to exchange this box's instance-role identity for a short-lived
# token impersonating oleum-devbox@oleum-claude-code. Tokens last ~1h and refresh automatically.
# This drives BOTH Claude-Code-on-Vertex and every google-cloud-* SDK / gcloud call.
export GOOGLE_APPLICATION_CREDENTIALS="$HOME/.config/gcloud/devbox-wif-credentials.json"
export CLOUD_ML_REGION=global #us-east5
export ANTHROPIC_VERTEX_PROJECT_ID=oleum-claude-code
# Deliberately NOT setting GOOGLE_CLOUD_PROJECT here. It is read by every google-cloud-* client
# as a silent global default, so a shell-wide value would make an un-parameterized script write to
# whichever project the profile last named. Pass the project explicitly per call
# (`--project`, `storage.Client(project=...)`, `aiplatform.init(project=...)`), or scope it to one
# shell/dir with CLOUDSDK_CORE_PROJECT / GOOGLE_CLOUD_PROJECT when working in another project.
# AWS settings
...
if [[ "$CLAUDE_PROVIDER" == "vertex" ]]; then
  ...
  export ANTHROPIC_DEFAULT_FABLE_MODEL="global.anthropic.claude-fable-5-1[1m]"
